import * as captcha from './2captcha.js';
...
const solve = async (response) => {
  ...
  if (sitekey && sitekey[1]) {
    ...
    if (captchaMethod === 'cf_captcha_kind=h' || captchaMethod === 'cf_captcha_kind=g') {
      const { rucaptcha, captchaKey, onCaptcha } = response.request.options.context;
      ...
      const captchaResponse = onCaptcha ? await onCaptcha(captchaOptions) : await captcha.solve({ ...captchaOptions, rucaptcha });
