  ...
  if (sitekey && sitekey[1]) {
    ...
    if (captchaMethod === 'cf_captcha_kind=h' || captchaMethod === 'cf_captcha_kind=g') {
      const captchaOptions = {
        rucaptcha: response.request.options.rucaptcha,
        key: response.request.options.captchaKey,
        pageurl: response.url,
        sitekey: sitekey[1],
        method: captchaMethod.split('=')[1] + 'captcha',
      };
      const captchaResponse = response.request.options.onCaptcha
        ? await response.request.options.onCaptcha(captchaOptions)
        : await captcha.solve(captchaOptions);
