...
const solveCaptcha = async ({ key, pageurl, sitekey, method = "h" }) => {
  // Post captcha information
  console.log(`Solving ${method} at ${pageurl}`);
  ...
  while (maxAttempt > 0) {
    ...
    const { body } = await got.get(
      `https://2captcha.com/res.php?key=${key}&action=get&id=${data.request}&json=1`
    );
